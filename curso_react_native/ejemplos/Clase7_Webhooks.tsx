...
export default function Clase7WebhooksEjemplo() {
  ...
  const obtenerConfiguracionEvento = (tipo: string) => {
    switch (tipo) {
      case 'star':
        return { emoji: '⭐', titulo: 'New Star!', color: '#fbbf24', bg: '#2d2206' };
      case 'issue':
        return { emoji: '🐛', titulo: 'New Issue', color: '#ef4444', bg: '#2d0606' };
      case 'push':
        return { emoji: '🚀', titulo: 'New Commit', color: '#3b82f6', bg: '#061c2d' };
      case 'pull_request':
        return { emoji: '🔀', titulo: 'Pull Request', color: '#10b981', bg: '#062d18' };
      default:
        return { emoji: '❓', titulo: 'Evento GitHub', color: '#9ca3af', bg: '#1f2937' };
    }
  };
    ...
    return (
      <View style={[styles.card, { borderColor: config.color, backgroundColor: config.bg }]}>
        <View style={styles.cardHeader}>
          ...
          <Text style={[styles.cardTitle, { color: config.color }]}>{config.titulo}</Text>
          <Text style={styles.cardTime}>{fechaFormat}</Text>
        </View>
        <Text style={styles.cardDetails}>
          <Text style={styles.boldText}>{item.actor}</Text> actuó en el repositorio{' '}
          <Text style={styles.boldText}>{item.repo}</Text>
        </Text>
      </View>
    );
  };
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            {estadoConexion === 'CONECTANDO' ? (
              ...
              <Text style={styles.emptyText}>Esperando eventos de GitHub...</Text>
            )}
          </View>
        }
      />
